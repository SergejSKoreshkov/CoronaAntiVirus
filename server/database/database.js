...
exports.initHistory = () => new Promise((resolve, reject) => {
    db.run('CREATE TABLE IF NOT EXISTS history (id INTEGER PRIMARY KEY AUTOINCREMENT, gate_id TEXT NOT NULL, card_id TEXT NOT NULL, temp TEXT NOT NULL, time TIME NOT NULL)', (err) => {
        if (err) reject(err)
        else resolve()
    })
})
...
exports.addHistory = (values = []) => new Promise((resolve, reject) => {
    db.run(`INSERT INTO history (gate_id, card_id, temp, time) VALUES ('${values.join('\',\'')}')`, (err) => {
        if (err) reject(err)
        else resolve()
    })
})
